...
All AI logic is in analyzer.py (CLIP embeddings + nearest-neighbor/classifier).
"""
...
def analyze():
    ...
    Form fields:
      ...
      mode                — 'nn' | 'classifier'  (default: nn)
      context_multiplier  — optional float, e.g. 1.3 for a restricted zone
                            or nighttime event (default: 1.0)
    """
    ...
    mode = request.form.get("mode", "nn")
    if mode not in ("nn", "classifier"):
        mode = "nn"

    try:
        context_multiplier = float(request.form.get("context_multiplier", 1.0))
    except ValueError:
    ...
    t0 = time.time()
    try:
        result = analyze_image(str(save_path), mode=mode, context_multiplier=context_multiplier)
    except Exception as e:
...
def api_status():
    ...
    return jsonify({
        ...
        "classifier_available":  classifier_available,
        "engines": {
            "nearest_neighbor": "clip-vit-base-patch32" if embeddings_available else "none",
            "classifier":       "logistic-regression"   if classifier_available else "none",
        }
    })
...
if __name__ == "__main__":
    ...
    log.info("╔══════════════════════════════════╗")
    log.info("║  SENTINEL v3.0 — Starting        ║")
    log.info("╚══════════════════════════════════╝")
    log.info(f"  Dataset embeddings : {'✅' if embeddings_available else '❌ (run embed_dataset.py)'}")
    log.info(f"  Trained classifier : {'✅' if classifier_available else '❌ (run train_classifier.py, or use mode=nn)'}")
